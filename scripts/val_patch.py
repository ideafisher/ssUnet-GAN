...
def save_image_color_masking(output_folder, image_name, full_img, all_class_mask,  gt_class_mask, config, gt_mask_flag):
    for c in range(config['num_classes']):
        ...
        if c > 0:
            if gt_mask_flag == True:
                save_name = os.path.join(output_folder, config['name'], file_name + '_GT_RE_masking_p075.jpg')
                save_name_GT = os.path.join(output_folder, config['name'], file_name + '_GT_masking_p075.jpg')
            save_name_RE = os.path.join(output_folder, config['name'], file_name + '_RE_masking_p075.jpg')
...
def main():
    ...
    with torch.no_grad():
        for full_img, full_label, input, target, image_name in tqdm(val_loader, total=len(val_loader)):
            ...
            for c in range(config['num_classes']):
                ...
                if c > 0:
                    ...
                    save_masking_GT(np.array(save_img), mask_gt, mask_output, save_name_GT)
                    save_img = full_img.copy()
                    save_masking_RE(np.array(save_img), mask_gt, mask_output, save_name_RE)
                    save_img = full_img.copy()
                    save_masking(np.array(save_img), mask_gt, mask_output, save_name)
