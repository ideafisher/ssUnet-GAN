...
def patch_gen_img_mask(img_mask,  p_size):

    img_h = img_mask.shape[0]
    img_w = img_mask.shape[1]
    overlap = 0.5
    i_w = int(math.floor(img_w / (overlap * p_size)))-1
    i_h = int(math.floor(img_h / (overlap * p_size)))-1
    ...
    for i in range(i_w):
        for j in range(i_h):
            ...
            idx_h2 = int(idx_h1 + p_size)
            #print(idx_w1, idx_w2, idx_h1, idx_h2)
            image_patch.append(img_mask[idx_h1:idx_h2, idx_w1:idx_w2,:])
...
def patch_gen_mask(img_mask,  p_size):
...
def save_image_patch(dataset_node, image_patch, img_size, image_name, data_cnt):
    for pidx in range(len(image_patch)):
        p_image = image_patch[pidx]
        # p_mask = mask_patch[pidx]
        tmp_data_cnt = data_cnt + pidx
        # resized_img = cv2.resize(img, (img_size, img_size))
        file_name = '{:s}_{:05d}'.format(image_name, tmp_data_cnt) + '.png'
...
def save_label_patch(dataset_node, mask_patch, img_size, class_idx,  image_name, data_cnt):
    for pidx in range(len(mask_patch)):
        p_mask = mask_patch[pidx]
        # p_mask = mask_patch[pidx]
        tmp_data_cnt = data_cnt + pidx
        # resized_img = cv2.resize(img, (img_size, img_size))
        ...
        path = os.path.join('../inputs/{:s}_{:d}/'.format(image_name, img_size), 'annotations', dataset_node,
                            str(class_idx), file_name)
        single_label = resized_mask > 0
        all_mask[single_label] = class_idx
        cv2.imwrite(path, resized_mask)
        #all_mask_path = os.path.join('../inputs/{:s}_{:d}'.format(image_name, img_size), 'annotations', dataset_node, file_name)
        #all_mask = (all_mask * 1).astype('uint8')
        #cv2.imwrite(all_mask_path, all_mask)
...
def save_image_mask(image_paths, dataset_node, image_name, num_class, img_size):
    ...
    for i in tqdm(range(len(image_paths))):
        ...
        i_dir = os.path.dirname(img_path)
        label_path = i_dir.replace('image','label')
        ...
        image_patch = patch_gen_img_mask(img, p_size)
        if 1:
            for c_idx in range(num_class):
                label_dir = os.path.join(label_path, str(c_idx),i_name )
                label_dir = label_dir.replace('img','label')
                mask_or = cv2.imread(label_dir, cv2.IMREAD_GRAYSCALE)
                #resized_mask = cv2.resize(mask_or, (img_size, img_size))
                resized_mask = post_process_resized_mask(mask_or)

                mask_patch = patch_gen_mask(resized_mask, p_size)
                if len(image_patch) !=  len(mask_patch):
                    print('patch length error ={:s}'.format(image_name))
                save_label_patch(dataset_node, mask_patch, img_size, c_idx, image_name, data_cnt)
        save_image_patch(dataset_node, image_patch, img_size, image_name, data_cnt)

        data_cnt += len(image_patch)
...
def main():
    ...
    image_name = 'SN7'
    image_paths = glob('../inputs/{:s}/image/*.png'.format(image_name))
